...
function Funds() {
    ...
    useEffect(() => {
        fetchFunds();
    }, [updateFund, deleteFund, createNewFund])
    
    useEffect(() => {
        const fundsFiltered = funds.filter(fund => 
            fund.title.toLowerCase().includes(searchTitle.toLowerCase())
        );
        const fundsNotReachedGoal = fundsFiltered.filter(fund => fund.funded > fund.fund_goal/fund.funded*100);
        const fundsReachedGoal = fundsFiltered.filter(fund => fund.funded <= fund.fund_goal/fund.funded*100);
        const fundsSorted = [...fundsNotReachedGoal].reverse();
        const combinedFunds = [...fundsReachedGoal, ...fundsSorted];
